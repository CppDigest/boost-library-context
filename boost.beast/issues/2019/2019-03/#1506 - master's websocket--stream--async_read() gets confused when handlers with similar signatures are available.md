# #1506 - master's websocket::stream::async_read() gets confused when handlers with similar signatures are available [Closed]

> Username: reddwarf69  
> Created at: 2019-03-07 05:58:23 UTC  
> Updated at: 2019-03-10 21:57:11 UTC  
> Closed at: 2019-03-10 21:57:10 UTC  
> Url: https://github.com/boostorg/beast/issues/1506  

Using Boost 1.69 this example compiles. But using Boost `b2e37d03b4aa23f8add2ce3dd37d3041c19591d2` it fails with:  
  
```  
/include/boost/beast/core/async_base.hpp:351:13: error: call of ‘(echo_op<boost::beast::websocket::stream<boost::asio::basic_stream_socket<boost::asio::ip::tcp> >, main(int, char**)::<lambda(boost::beast::error_code)> >) (boost::asio::error::basic_errors, int)’ is ambiguous  
             h_(std::forward<Args>(args)...);  
             ^~  
echo_op.cpp:199:6: note: candidate: ‘void echo_op<AsyncStream, Handler>::operator()(boost::beast::error_code, std::size_t) [with AsyncStream = boost::beast::websocket::stream<boost::asio::basic_stream_socket<boost::asio::ip::tcp> >; Handler = main(int, char**)::<lambda(boost::beast::error_code)>; boost::beast::error_code = boost::system::error_code; std::size_t = long unsigned int]’  
 void echo_op<AsyncStream, Handler>::  
      ^~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
echo_op.cpp:188:10: note: candidate: ‘void echo_op<AsyncStream, Handler>::operator()(boost::beast::error_code, bool) [with AsyncStream = boost::beast::websocket::stream<boost::asio::basic_stream_socket<boost::asio::ip::tcp> >; Handler = main(int, char**)::<lambda(boost::beast::error_code)>; boost::beast::error_code = boost::system::error_code]’  
     void operator()(boost::beast::error_code ec, bool test)  
```  
  
Diff from echo_op example from Boost 1.69  
```  
@@ -8,6 +8,7 @@  
 //  
...
+            return p.stream.async_read(p.buffer, std::move(*this));  
+            //return boost::asio::async_read_until(p.stream, p.buffer, "\r", std::move(*this));  
...
+                init.completion_handler}(boost::beast::error_code{}, std::size_t {0});  
...
-    async_echo(sock,  
+  
+    boost::beast::websocket::stream<socket_type> ws(ioc);  
+  
+    async_echo(ws,  
...
> Username: djarek  
> Created at: 2019-03-10 20:58:22 UTC  
> Url: https://github.com/boostorg/beast/issues/1506#issuecomment-471343819  
...
> Created at: 2019-03-10 21:00:11 UTC  
> Url: https://github.com/boostorg/beast/issues/1506#issuecomment-471343975  

Please confirm this is fixed in **develop** and then close the issue, thanks!
...
> Username: reddwarf69  
> Created at: 2019-03-10 21:57:10 UTC  
> Url: https://github.com/boostorg/beast/issues/1506#issuecomment-471358029  
